...
export const KnowledgeBase = () => {
  ...
  const handleUpload = (e) => {
    ...
    addContextDoc({ ...newDoc, content: newDoc.content || `Content for ${newDoc.name}`, targetId: selectedId, targetType: view });
    setShowUpload(false);
    setNewDoc({ name: '', type: 'Protocol', size: '1.5MB', content: '' });
  };
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleUpload} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-zinc-500 mb-1.5 block tracking-widest">Content</label>
                <textarea required placeholder="Enter context document content..." rows={4} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-indigo-500/50 resize-none" value={newDoc.content || ''} onChange={e => setNewDoc({ ...newDoc, content: e.target.value })} />
              </div>
